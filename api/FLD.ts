...
const knownFlashLoanProviders: Record<string, string> = {
...
const flashLoanEventSignatures: Record<string, string> = {
...
export async function detectFlashLoanAttack(blockNumber: number) {
    ...
    for (const tx of block.transactions) {
        ...
        const { isFlashLoan, matchedProvider, matchedEvent } = await detectFlashLoanInTransaction(tx);
...
async function getCurrentEthPrice(): Promise<number> {
    try {
        const response = await axios.get(`https://eth-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_API_KEY}/getTokenPrice`, {
            params: {
                contractAddress: "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE" // ETH contract address
            }
        });
        return response.data.tokenPrice?.usd || 2000; // Default to 2000 if API fails
    } catch (error) {
...
async function detectFlashLoanInTransaction(
    tx: ethers.providers.TransactionResponse,
    maxRetries = 3,
    delayMs = 100
): Promise<{ isFlashLoan: boolean; matchedEvent?: string; matchedProvider?: string; }> {
    ...
    while (attempt < maxRetries) {
        try {
            ...
            for (const log of receipt.logs) {
                ...
                if (matchedEvent || matchedProvider) break;
            }
